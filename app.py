...
# --- EXTRAÇÃO MELHORADA ---
uploaded_file = st.file_uploader("Upload do Print (Placas ou Ilhas)", type=["jpg", "png", "jpeg"])

if uploaded_file:
    img = Image.open(uploaded_file)
    if st.button("🔍 EXTRAIR E SINCRONIZAR"):
        with st.spinner("Processando imagem e buscando caracteres..."):
            resultados = reader.readtext(np.array(img))
            
            # Criamos uma lista organizada do que foi lido
            texto_lista = []
            for res in resultados:
                texto_lista.append(res[1].upper().strip())

            padrao_placa = re.compile(r'[A-Z]{3}[0-9][A-Z0-9][0-9]{2}')

            # 1. Busca pelas Ilhas (Mantendo caracteres como >)
            for i, texto in enumerate(texto_lista):
                for rota_id in st.session_state.dados_controle.keys():
                    if rota_id in texto:
                        # Se encontrou o XPT, olha até 2 posições antes para achar a Ilha
                        for busca in range(1, 3):
                            if i - busca >= 0:
                                val_anterior = texto_lista[i-busca]
                                # Verifica se o valor é uma letra (ex: A) ou Caractere+Letra (ex: >A)
                                # O regex busca opcionalmente um símbolo seguido de uma letra única
            ...
            # 2. Busca pelas Placas
            current_xpt = None
            for texto in texto_lista:
                texto_limpo = texto.replace(" ", "")
                # Identifica a rota atual no scroll da imagem
                for rota in st.session_state.dados_controle.keys():
                    if rota in texto_limpo: 
                        current_xpt = rota
                
                # Se achar placa, vincula ao último XPT detectado
...
for rota in list(st.session_state.dados_controle.keys()):
    ...
    with st.expander(f"📍 {rota} - {info['local']} (Ilha: {info['letra']})", expanded=True):
        ...
        info['letra'] = c_letra.text_input("Ilha", info['letra'], key=f"l_{rota}", on_change=salvar_no_firebase, args=(st.session_state.dados_controle,))
        info['janela'] = c_hora.text_input("Hora", info['janela'], key=f"h_{rota}", on_change=salvar_no_firebase, args=(st.session_state.dados_controle,))
        
        if c_add.button("➕ Placa", key=f"btn_a_{rota}"):
...
for rota, info in st.session_state.dados_controle.items():
    if info['veiculos']:
        ...
        for v in info['veiculos']:
            status_icon = "✅" if "FINALIZADO" in v['status'] else "🚚"
            res_texto += f"{status_icon} {v['placa']} - {v['status']}\n"
        res_texto += "\n"

if tem_dados:
    st.divider()
    st.text_area("Texto para Copiar", res_texto, height=200)
